...
var UserAccountWhere = struct {
	...
	Description: whereHelperstring{field: "\"user_account\".\"description\""},
	UserType:    whereHelperstring{field: "\"user_account\".\"user_type\""},
	UpdateTime:  whereHelpertime_Time{field: "\"user_account\".\"update_time\""},
	CreateTime:  whereHelpertime_Time{field: "\"user_account\".\"create_time\""},
}

// UserAccountRels is where relationship names are stored.
var UserAccountRels = struct {
	UserCustomerOrders string
}{
	UserCustomerOrders: "UserCustomerOrders",
}

// userAccountR is where relationships are stored.
type userAccountR struct {
	UserCustomerOrders CustomerOrderSlice
}
...
var (
	userAccountAllColumns            = []string{"id", "name", "password", "email", "description", "user_type", "update_time", "create_time"}
	userAccountColumnsWithoutDefault = []string{"name", "password", "email", "description"}
	userAccountColumnsWithDefault    = []string{"id", "user_type", "update_time", "create_time"}
...
func (q userAccountQuery) One(ctx context.Context, exec boil.ContextExecutor) (*UserAccount, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "model: failed to execute a one query for user_account")
	}
...
func (q userAccountQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to count user_account rows")
	}
...
func (q userAccountQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "model: failed to check if user_account exists")
	}

	return count > 0, nil
}

// UserCustomerOrders retrieves all the customer_order's CustomerOrders with an executor via user_id column.
func (o *UserAccount) UserCustomerOrders(mods ...qm.QueryMod) customerOrderQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"customer_order\".\"user_id\"=?", o.ID),
	)
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"customer_order\".*"})
	}

	return query
}

// LoadUserCustomerOrders allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userAccountL) LoadUserCustomerOrders(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUserAccount interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`customer_order`), qm.WhereIn(`customer_order.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load customer_order")
	}

	var resultSlice []*CustomerOrder
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice customer_order")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on customer_order")
	}
	...
	if singular {
		object.R.UserCustomerOrders = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.UserCustomerOrders = append(local.R.UserCustomerOrders, foreign)
				if foreign.R == nil {
					foreign.R = &customerOrderR{}
				}
...
// AddUserCustomerOrders adds the given related objects to the existing relationships
// of the user_account, optionally inserting them as new records.
// Appends related to o.R.UserCustomerOrders.
// Sets related.R.User appropriately.
func (o *UserAccount) AddUserCustomerOrders(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*CustomerOrder) error {
	var err error
	...
	if o.R == nil {
		o.R = &userAccountR{
			UserCustomerOrders: related,
		}
	} else {
		o.R.UserCustomerOrders = append(o.R.UserCustomerOrders, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &customerOrderR{
				User: o,
			}
		} else {
			rel.R.User = o
...
func FindUserAccount(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*UserAccount, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "model: unable to select from user_account")
	}
...
func (o *UserAccount) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("model: no user_account provided for insertion")
	}
	...
	if err != nil {
		return errors.Wrap(err, "model: unable to insert into user_account")
	}
...
func (o *UserAccount) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("model: unable to update user_account, could not build whitelist")
		}
	...
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to update user_account row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to get rows affected by update for user_account")
	}
...
func (q userAccountQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to update all for user_account")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to retrieve rows affected for user_account")
	}
...
func (o *UserAccount) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("model: no user_account provided for upsert")
	}
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("model: unable to upsert user_account, could not build update column list")
		}
	...
	if err != nil {
		return errors.Wrap(err, "model: unable to upsert user_account")
	}
...
func (o *UserAccount) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to delete from user_account")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to get rows affected by delete for user_account")
	}
...
func (q userAccountQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to delete all from user_account")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to get rows affected by deleteall for user_account")
	}
...
func UserAccountExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "model: unable to check if user_account exists")
	}
